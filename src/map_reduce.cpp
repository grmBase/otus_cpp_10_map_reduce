...
int t_map_reduce::do_work()
{
  ...
  // TODO: как некий "заранее дебаг" в нашей версии ещё можно добавить проверку на не повторение строк в файле 
  // Иначе всё равно не найдём ничего. Но в общей случае в реальном map-reduce мы этого конечно знать не можем


  // Ниже получаю максимальну длину строки исходного файла. Вероятно это конечно не по "map-reduce-ски", но
  // наверно для домашнего задания пойдёт. К тому же именно такой шаблон был предоставлен в качестве примера


  // Если же такая задача будет в реальности, и "33 раза" делать проходы не хорошо, то думаю нужно пробовать сделать на 
  // стадии map из каждой строки генерить все записи и с префиком 1 буква и 2 и 3, и так до конца всех символов строки. 
  // а потом уже после стадии редюса искать минимальную, где уже нет повторений
